...
#detail : 글 상세보기 페이지
#DetailView : 특정 모델클래스 객체 한개를 추출해서
#템플릿에 넘겨주는 제네릭뷰

#DetailView는 request외에 추가 매개변수를 사용해
#추출할 객체를 찾음
#-> pk : id값 같은 고유값으로 객체를 찾을때 사용하는 변수
...
class Posting(LoginRequiredMixin ,FormView):
    ...
    #사용자가 준 추가데이터(이미지,파일정보)를 처리하기위해서
    #해당 함수를 오버라이딩
    ...
    def form_valid(self, form):
        ...
        #request.FILES : 클라이언트가 서버로 보낸 파일정보를
        #관리하는 변수 (객체)
        
        #f : 사용자가보낸 이미지파일의 실제정보
